...
var _bzz_config: ApiApiConfigClient:
	set(value):
		_bzz_config = value
	get:
		if not _bzz_config:
			_bzz_config = ApiApiConfigClient.new()
		return _bzz_config
...
func _bzz_do_request_text(
	...
	var requesting := self._bzz_client.request(method, path_queried, headers_for_godot, "" if body_serialized == "null" else body_serialized)
	if requesting != OK:
		var error := ApiApiErrorClient.new()
	...
	# Perhaps use a method of ApiApiResponseClient for this?
	var encoding := "utf-8"
